...
example will be outrageously complex in S₀, for a few reasons that combine
together:
...
This call gets "translated" into an S₀ invocation.  In this case, we're invoking
the `+` branch of the value named `six` in the current environment.
...
(There is additional syntax that gives you more control over how the
continuation is passed in to the invocation, but we'll ignore that for now.)
