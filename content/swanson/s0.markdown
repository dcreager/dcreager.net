...
<div class="aside" markdown=1>
How are some ways that S₀'s syntax is simple?  As just one example, lines in an
S₀ file **_must_** end specifically with a newline character (decimal 10 in
ASCII, `\n` in C-like languages).  Other line endings are _not_ allowed.
</div>
...
As mentioned in the previous post, Swanson names are _binary_.  There are two
...
Each block starts with `containing` and `receiving` clauses, which define which
...
Can you see why this isn't a language that you'd want to program in directly?
